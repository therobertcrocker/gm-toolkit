...
func TestApplyBookkeeping_Income(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			newMutationEngine().Apply(s, mutations)
...
func TestApplyBookkeeping_NoDoubleApply(t *testing.T) {
	...
	mutationEngine.Apply(s, mutations)

	_, secondMutations, _ := te.ApplyBookkeeping(s) // second call should be a no-op
	if len(secondMutations) != 0 {
		t.Errorf("second ApplyBookkeeping returned %d mutations, want 0", len(secondMutations))
	}
	mutationEngine.Apply(s, secondMutations)
