...
| [refactor/ability-engine-redesign](#refactorability-engine-redesign) | 216–217 | Ability engine retire: TOML decoder wiring for flat fields; `SelectMoveDestination` removal from action.Collector |
| [feature/movement-redesign — spatial interface](#featuremovement-redesign--spatial-interface) | 218–226 | Spatial interface narrowing, RegionHex routing type, HexRouter consumer contract, MovementOrder.Path domain change, World/Location RegionHex unification |
| [feature/movement-redesign — engine (Effort 2)](#featuremovement-redesign--engine-effort-2) | 227–235 | Movement phase orchestration, in-flight revision pathing, mutation emit-site correctness, LockSkip test strategy, transport cargo collection boundary, EffectsEngine data-driven registration, two-dispatch movement phase, RegisterHooks rename, dispatch trigger-batch semantics |
| [feature/movement-redesign — cutover (Effort 3)](#featuremovement-redesign--cutover-effort-3) | 236–238 | TOML migration: ExcludeCategories two-axis cargo filter, ChangeHomeworld action initiation (D112 reversed), hex-scoped attack targeting universally |
| [feature/movement-redesign — pre-merge checklist](#featuremovement-redesign--pre-merge-checklist) | 239–240 | MutationEngine fail-loud on missing entities: accumulator semantics; ScavengersReactor guard removal |
| [feature/logging](#featurelogging) | 241–250 | Package location, branch split, tool param, banner shapes, dynamic-rail sizing, LogsDir in config, version literal, turn=cycle, two-channel logger, engine attr binding site |
| [feature/errors](#featureerrors) | 251–254 | recoverable package location, inline vs helper, OnFactionSkipped on recoverable skip, nil-return correctness |
| [feature/tui-foundation](#featuretui-foundation) | 255–261 | Adapter cfg field; styles sub-package; no Mode alias; dry-run inline rulebook; smokeHexRouter; pre-merge cleanup |
...
| [feature/tui-manage](#featuretui-manage) | 274–284 | Message sub-package for import-cycle avoidance; detail viewport scrolling; permanent help footer; root-owned layout budget; key-routing capability predicate; atomic CRUD mutations; `NewFaction` precondition panic; `contextstrip` deleted |
| [feature/tui-turn](#featuretui-turn) | 285–294 | Execution overlay scoped to the center region (revises plan Decision 11); TUI world-engine wiring reuse + deferred constructor restructure (R-012); overlay message forwarding + Init cmd propagation; quit tab removed; center-column stream/banner/detail-card phase anchoring; single-phase self-registering engine construction (resolves R-012); Esc-declines for movement/cargo overlays |
...
| 165 | `dispatchMutationReactors` passes only the newly-emitted `newMutations` slice to each recursive depth; the accumulated result is built on the way back via `append(combined, deeper...)` | Originally specified as "full combined slice" with reactor idempotency as the guard. Revised in feature/movement-redesign when `TransportReactor` (stateless, fires on every `MovementOrderIssued` it sees) proved that idempotency cannot compensate: the same trigger appeared in every depth's input and exhausted the recursion cap. Trigger-batch semantics are the correct design for stateless reactors. See Decision 235. |
...
### refactor/ability-engine-redesign

| # | Decision | Rationale |
|---|----------|-----------|
| 216 | `abilityRecord` in `rulebook/rulebook.go` extended with flat `toml:` tags (`attacker_stat`, `defender_stat`, `effect`); `convertAbility` updated to populate the corresponding `AbilityDefinition` fields | R-001's plan migrated Informers' TOML from `[[steps]]` to flat `[ability]` keys but did not explicitly update the TOML decoder record. Execution revealed the gap: the TOML library decodes silently into zero values when no matching struct tag exists. Flat fields must be declared on the intermediate record just as `[[steps]]` entries were on `abilityStepRecord`. |
| 217 | `SelectMoveDestination` removed from `action.Collector`; `SelectFactionTestTarget` inlined directly (no longer via `ability.Collector` embedding) | Deleting `internal/faction/engine/ability/` removed `ability.Collector`, which `action.Collector` embedded. `SelectMoveDestination` was only ever called by the now-deleted `steps.Movement` handler — no action handler calls it post-R-001, so it is dropped from the interface. `SelectFactionTestTarget` is still required by the Informers handler and is inlined directly on `action.Collector`. |

<br />

### feature/movement-redesign — spatial interface

| # | Decision | Rationale |
|---|----------|-----------|
| 218 | `HybridMap` renamed to `RegionMap`; constructor `LoadHybrid` → `LoadRegionMap`; file `hybrid_map.go` → `region_map.go` | "HybridMap" described implementation history (hex grid + graph edges combined); "RegionMap" describes what the type *is* — a spatial map organized into named regions with hex grids and boundary connections. |
| 219 | `SpatialMap` narrowed to `Location(id string) (Location, bool)` only; `Distance` and `Path` removed from the interface | The old `SpatialMap` was dishonest: pure graph or voxel maps cannot implement `Path([]HexCoord)` or `Distance(crossingCost int)` — those are `RegionMap`-specific concepts. Future map kinds define their own routing API to match their shape. `HexMap` and `GraphMap` stubs deleted. |
| 220 | `RegionHex{RegionID string, Coord HexCoord}` defined in `region_map.go` (not `spatial.go`) as the routing input/output type | Hex coordinates are local to regions — the same `(Q,R)` can appear in two different regions. Bare `HexCoord` is ambiguous as a routing input. `RegionHex` bundles region and coord so routing is unambiguous without a reverse-lookup. It lives alongside `RegionMap` because it is a `RegionMap`-specific routing concept, not a universal spatial contract. |
| 221 | `RegionMap.Path` returns `[]RegionHex` (not `[]HexCoord`); `RegionOfHex` removed from all interfaces | Because each step in the returned path carries its `RegionID`, consumers never need a reverse-lookup. `TickMovementOrders` reads `step.RegionID` directly. `RegionOfHex` remains as an unexported-adjacent utility on `*RegionMap` but is not on any interface. |
| 222 | `world.HexRouter` defined in `internal/faction/engine/world/` as the consumer-side interface; compile-time assert `var _ HexRouter = (*spatial.RegionMap)(nil)` lives there | Consumer-driven interface design: `world` declares exactly the shape it requires rather than `spatial` declaring what all consumers need. The spatial library stays honest about what it universally promises; the engine owns the contract it actually exercises. |
| 223 | `domain.MovementOrder.Path` changed from `[]spatial.HexCoord` to `[]spatial.RegionHex` | Path steps must carry region identity so the mutation engine and tick loop can label movement events (`MovementOrderProgressed.Region`) without an ambiguous reverse-lookup. This is a domain type change; state persistence handles it via the existing TOML/JSON tags. |
| 224 | `spatial.World` stores `loc RegionHex` instead of flat `region string` + `hex HexCoord` fields; `RegionHex() RegionHex` method added | A world IS at a `RegionHex` — the internal representation should match the concept. The method gives callers direct routing input, eliminating manual assembly from `Coords()` + `RegionID()` at every call site. |
| 225 | `spatial.RegionLocation` interface extended with `RegionHex() RegionHex` | Every `RegionLocation` already exposes the components of a `RegionHex` via `Coords()` and `RegionID()`, but callers had to assemble them manually. Adding `RegionHex()` to the interface makes the common operation a single call. `World` is the only declared implementor, so the contract extension is low-risk. |
| 226 | `domain.Location` reshaped: flat `HexCoords spatial.HexCoord` + `Region string` replaced by `RegionHex spatial.RegionHex`; `WorldID string` retained | An asset's position is always a `RegionHex`; the flat fields duplicated what `RegionHex` already encodes. `WorldID` is kept separately because `RegionHex` alone cannot distinguish "anchored at a named world" from "mid-journey at an anonymous hex" — `WorldID == ""` signals in-transit. |

<br />

### feature/movement-redesign — engine (Effort 2)

| # | Decision | Rationale |
|---|----------|-----------|
| 227 | `runMovementPhase` placed before the `LockSkip` early-exit in `RunFactionTurn`, not inside the LockSkip branch | Movement is a bookkeeping-adjacent phase — in-flight orders tick regardless of goal lock. Inserting before the branch avoids duplicating the call and makes the intent explicit: LockSkip gates only action selection, not movement. |
| 228 | Task 10 (mid-flight revision pathing) resolved without `PathFromHex`: `asset.Location.RegionHex` used directly as the path origin | The plan listed `PathFromHex` as the preferred option, but the `domain.Location` RegionHex unification (Decision 226) made it unnecessary — an in-flight asset's `Location.RegionHex` is always the correct routing origin. No interface change required. |
| 229 | `MovementOrderIssued`, `MovementOrderRevised`, and `MovementOrderCancelled` must explicitly set `FactionID` and `AssetID` at the emit site in `BuildMovementMutations` | `mutation.go`'s Apply branches key on these fields to locate the correct faction/asset in state. Emitting without them is a silent no-op: the mutations are recorded to history but state is never updated. Discovered during Commit 4 test authoring when post-Apply state assertions failed. |
| 230 | LockSkip movement scenario tests combine post-Apply state inspection with persisted-history checks | State inspection (`CurrentOrder.StepIdx`, `Location.RegionHex`) proves the Apply branch ran; history checks (`FindMutationByTypeAndCause`) prove the mutation was emitted and persisted. Each catches a distinct bug class — the emit-site fix in Decision 229 would have been caught by state inspection but not by observer-payload-only assertions. |
| 231 | `SelectTransportCargo` called in `prepareMovementDecisions`, not inside `BuildMovementMutations` | The plan placed cargo collection inside the engine method, but that violated the collector-at-the-boundary pattern: `BuildMovementMutations` is a pure data transform (decisions in, mutations out) and must not perform I/O. `prepareMovementDecisions` is already the orchestrator-level seam where all collector calls live. Cargo IDs are pre-populated on `MovementDecision.CargoAssetIDs` before `BuildMovementMutations` is called; the engine method reads them as plain data. |
| 232 | `EffectsEngine` mirrors `TagEngine` but registration is data-driven from the rulebook, not code-driven | `tag.New()` hard-codes its handler set because tag behavior is static (Scavengers always means +1 Coin per kill). Transport behavior is parameterized by `TransportProfile`, which is data. The registration loop in `NewWithRulebook` walks `rulebook.Assets` and registers one `TransportHandler` per def with `Transport != nil`. Adding a new transport asset requires zero Go code changes — just a `[transport]` block in the rulebook. |
| 233 | `runMovementPhase` invokes `dispatch.MutationReactors` twice — once per mutation batch (tick, decisions) — rather than the plan's single dispatch on decisions only | The plan only specified dispatch around `decisionMutations`, but the transport reactor must also fire on `MovementOrderProgressed` and `MovementOrderCompleted`, which originate in `tickMutations`. Single-batch dispatch would leave cargo orphaned on every tick. The shape can't be merged into one batch because `tickMutations` must Apply before `prepareMovementDecisions` runs (decision eligibility reads `asset.CurrentOrder`). Two dispatches preserve the existing apply ordering while ensuring every reactor-eligible mutation gets a dispatch pass. |
| 234 | Testharness `RegisterTags` renamed to `RegisterHooks`; calls both `Tag.ApplyAll` and `Effect.ApplyAll` | `RegisterTags` was already a misnomer — it registered hooks, not tags. With the addition of `EffectsEngine`, a sibling `RegisterEffects` method would have required every test to call two methods, with the same ordering concern. One `RegisterHooks` call collapses both ApplyAll fan-outs at the test boundary. Production wiring (when a CLI/TUI driver materializes) will need its own hook-registration site, but the test API need not predict its shape. |
| 235 | `dispatch.MutationReactors` revised to trigger-batch semantics (Decision 165 superseded) | Integration tests for transport lifecycle revealed that passing the full accumulated `combined` slice caused `TransportReactor` to re-fire on `MovementOrderIssued` at every recursive depth, instantly exhausting the five-level cap. The fix passes only `newMutations` to the recursive call; the accumulated result is assembled on the way back out. All 27 pre-existing dispatch tests pass unchanged — stateful one-shot reactors are unaffected by this semantic change. |

<br />

### feature/movement-redesign — cutover (Effort 3)

| # | Decision | Rationale |
|---|----------|-----------|
| 236 | `ExcludeCategories []FactionStat` added to `TransportProfile`; category-level exclusions are a separate filter from `CargoTypes` type-level inclusions | `CargoTypes` filters by `AssetDefinition.Type` (e.g. "Starship", "Military Unit"). Assets also carry a `Category` field (`FactionStat`: Force/Cunning/Wealth). "Non-Force" and "non-Starship" are different axes — the former excludes by category, the latter by type — and cannot both be expressed by the same field. `ExcludeCategories` is a negative filter applied after the `CargoTypes` pass; a cargo candidate must satisfy both constraints to be eligible. |
| 237 | D112 reversed: ChangeHomeworld is initiated by a registered action (`ChangeHomeworld`), mirroring SeizePlanet→PlanetarySeizure (D113). | D112's rationale (Goal-Engine ownership of multi-turn state) is preserved — the action only emits `GoalInitiated` + `GoalPhaseAdvanced`; subsequent turns are managed by the goal handler's `CheckLock` per D122. The goal-engine-driven initiation pathway D112 implied was never built; the action shape is symmetric with the only other multi-turn goal in the codebase.
| 238 | Attack targeting is hex-scoped universally: `attackerHasTarget` and `Resolve` always use `eligibleDefendersAtHex`/`liveDefendersAtHex` regardless of whether the attacker has a `WorldID` set. | The Phase 7 plan branched on `WorldID != ""`, routing on-world attackers through `AssetsByLocation` and in-transit attackers through `AssetsByHex`. Ratified during Commit 3 execution: any asset at a hex — on-world or in-transit — can attack or be attacked by any other asset at that hex. Hex is the universal combat scope. `AssetsByLocation` is retained for other lookups (base redirect, rivals listing) but is no longer the attack-targeting path.

### feature/movement-redesign — pre-merge checklist

| # | Decision | Rationale |
|---|----------|-----------|
| 239 | `MutationEngine.Apply` returns `*MutationApplyError` (accumulator, option a) rather than failing fast on the first missing entity. | All valid mutations in a batch are applied before surfacing diagnostics. Fail-fast (option b) would leave state partially applied with no signal about remaining misses — harder to diagnose and potentially worse state than applying what can be applied. The orchestrator wraps the returned error as a hard turn failure (`fmt.Errorf("applying mutations: %w", err)`), keeping the failure-propagation decision at the call site rather than inside the engine. `MutationMiss` carries `MutationType`, `FactionID`, and `EntityID` (empty for faction-level misses) to produce actionable error messages. |
| 240 | `ScavengersReactor.credited` guard removed (option b). | The recursion guard was dead code: `reactDispatch` passes only newly-emitted mutations to each recursion level, so the same `AssetRemoved` can never reach the reactor twice within one dispatch. Keeping the map as a struct field caused cross-turn under-crediting — if an asset with the same ID was destroyed again in a later turn, the stale map entry would silently suppress the credit. Removing the map is both simpler and correct. |
...
| # | Decision | Rationale |
|---|----------|-----------|
| 241 | Package located at `internal/logging`, not `pkg/logging` as the plan originally specified. | `gm-toolkit` is a single Go module containing both `internal/faction` and `internal/spatial` tool trees. `internal/` enforces "this module only" at the compiler level, which matches the package's actual scope. `pkg/` would have signaled "intended for external import," which is misleading given there's no external consumer and likely never will be. |
| 242 | F-003 splits into two feature branches (`feature/logging`, `feature/errors`) rather than the plan's single `feature/logging-and-errors`. | Each branch carries its own pre-merge checklist, decisions-log section, and version bump assessment. The single-branch framing was load-bearing for the original plan but became inconsistent with the larger work shape once logging was clearly separable from errors. Two branches preserve the dependency ordering (logging first) without coupling the merge gates. |
| 243 | `RunHeader` takes a `tool` parameter (`tool, version, campaign string, factions int`) and the header banner renders `=== <tool> <version> ===`. | The original plan's signature omitted `tool`, but the package is shared across `internal/faction` and `internal/spatial`. Hard-coding "Faction-Manager" in the banner would break the contract that spatial gets the same logging surface. Explicit caller-supplied tool name is cheaper than any auto-detection scheme and self-documents in the log file. |
| 244 | Header banner has no closing rail (open-bottom asymmetric with the turn banner's full box). | The header is the file prologue; what follows it is a turn banner, not more header content. A closing rail would suggest "section ends" but the section never ends — every subsequent line is "inside" the run. The asymmetry is intentional and signals nesting depth visually. Trailing blank line gives visual breathing room before the first turn banner. |
| 245 | Turn-banner rail length is computed dynamically from `utf8.RuneCountInString(content)` rather than fixed at a constant width. | Faction names vary widely (short like "Acme", long like "Outer Rim Confederation"). Fixed width either overflows or floats the border awkwardly. Rune-counting (not byte-counting) is correct for the `·` separator (multibyte UTF-8) and any non-ASCII characters in faction names. Caveat: rune count ≠ display width for CJK and emoji — those would size the rail too narrow. For the SWN faction-name vocabulary this is not a present concern; if it becomes one, swap to `golang.org/x/text/width` or `runewidth`. |
| 246 | `LogsDir` lives on `config.Config` alongside `SpatialDataDir` and `FactionDataDir`, not hard-coded or inferred at the `internal/logging` level. | Callers construct a `*slog.Logger` via `logging.New(cfg.LogsDir, debug)` and inject it; `logging` never touches `config`. Consistent with the existing pattern that all data-path concerns are caller-owned. Static data consolidation rule (memory: feedback_static_data_consolidation) supports keeping path config in one place rather than spreading it across packages. |
| 247 | `version` for the run header is a hardcoded `"dev"` string literal at the orchestrator's `RunCycle` call site, not a new `internal/version` package or a `config.Config` field. | A package containing one constant is overkill; a config field defers the choice and adds plumbing for no current benefit. The literal is trivially find-and-replaceable when a real release pipeline (ldflags, BuildInfo) lands. YAGNI applies. |
| 248 | `turn` cascade attribute is `factionState.CycleNumber`, not a turn-within-cycle index. | Banners read "Turn N · Faction X" — each faction's turn within cycle N. Matches how the SWN rulebook talks about turns (a cycle is "Turn 3 of the campaign"; each faction acts within it). Avoids tracking a separate ordinal counter. |
| 249 | Sub-engine method log surface is the two-channel design from the plan: struct field (base logger) **and** parameter (orchestrator-enriched logger). Confirmed at implementation time despite the field being currently unused. | Engine structs are shared across factions and cycles, so the field can't be mutated per call (race + context leakage). The parameter carries the per-call cascade attrs (`turn`/`faction`/`phase`/`engine`); the field exists for future sub-engine-internal lines that have no orchestrator context (startup, registration). Collapsing to one channel was considered and rejected. |
| 250 | Orchestrator binds `engine` attribute at each sub-engine delegation site via `phaseLog.With("engine", "<name>")`, not at the sub-engine level. | Engine name is cascade context, not internal sub-engine state. The orchestrator already owns the cascade; threading the binding there keeps the sub-engines unaware of their own attr label. Mirrors how `phase` and `turn` are bound by the layer that knows them. |
...
| # | Decision | Rationale |
|---|----------|-----------|
| 251 | `IsRecoverable` promoted to `internal/faction/errors/` (exported) rather than unexported in `internal/faction/engine/recoverable.go` as the plan specified. | `engine/recoverable.go` would have imported `action` to reference `action.ErrNoSelection`. `engine` is `action`'s indirect ancestor; placing the helper there created an import cycle. A dedicated `internal/faction/errors/` package breaks the cycle cleanly: `action` declares sentinels, `errors` imports `action` to build the list, and `engine` imports `errors` to call `IsRecoverable`. Exporting the function follows from the package change — callers outside `engine` may legitimately need the check. |
| 252 | Inline `if factionErrors.IsRecoverable(err) {...}` at each site rather than the `surface` helper the plan proposed as a candidate. | The plan deferred the decision to after seeing three sites converted. In practice the inline block is 4–5 lines, the per-site logic (which logger, which return value) differs enough that a helper would have required per-caller overrides, and the explicit control flow is easier to audit. Helper would have saved ~40 lines; legibility cost outweighed the saving. |
| 253 | `OnFactionSkipped` does not fire when `Action.Run` returns `ErrNoSelection` (recoverable action skip). | The existing `selectedAction == nil` path (no action chosen, `Action.Run` never called) keeps its `OnFactionSkipped` call — that is a different flow. An `ErrNoSelection` return means `Action.Run` was called and the collector returned no selection during execution; `OnError` is the appropriate and only observer signal for that case. Firing `OnFactionSkipped` too would misrepresent the flow (an action *was* selected; it just couldn't complete its inputs). |
| 254 | Recoverable branch sites return `nil` after `observer.OnError`; several wip commits had erroneously returned `err`. | The semantic contract of the recoverable branch is "log, notify, continue" — returning `err` would propagate the error to the caller and abort the turn, defeating the purpose. The bug was caught during test-writing: `TestRunFactionTurn_RecoverableErrNoSelection` asserted `RunFactionTurn` returns `nil` and failed until all sites were corrected. |
...
| # | Decision | Rationale |
|---|----------|-----------|
| 255 | `cfg *config.Config` added to `Adapter` struct and `New()` constructor; `Run()` passes `a.cfg` to `engine.RunCycle`. | The plan's `run.go` showed a 3-arg `RunCycle` call, but the actual signature is `RunCycle(factionState, cfg, collectors, observer)`. Rather than keeping `Run()` as a panic stub (possible, since Foundation never exercises it from the TUI) or passing `nil` (risky — `cfg` is dereferenced in phase sub-engines), the cleanest fix was to thread `cfg` through the Adapter at construction time. The `New()` caller in Commit 4's `tui.Run()` passes `nil` for both `*engine.Engine` and `*config.Config` — Foundation's TUI stub doesn't dereference either until Initiative 2 wires real state. |
| 256 | Styles moved to `internal/faction/tui/styles/` sub-package (`package styles`) rather than staying in `package tui`. | The plan's `manage.go` and `modebar.go` imported `tui` for style vars, while `tui/model.go` imported `tui/views/manage` and `tui/views/modebar` — a cycle Go rejects at build time. The fix was a leaf sub-package: `tui/styles` has no imports from `tui` or `tui/views/*`, so the import DAG remains directed. `tui/view.go` imports `tui/styles` with the `styles.` qualifier. |
| 257 | `modebar.Mode` used directly in the root `Model` struct fields; the plan's `type Mode = modebar.Mode` alias was dropped. | Type aliases for re-export are an established anti-pattern in this codebase (D-feedback: prefer real package boundaries). The alias provided no cross-package ergonomic benefit — `model.go`, `update.go`, and `view.go` are all in `package tui` and import `modebar` anyway, so `modebar.Mode` is already local. |
| 258 | `RunDryRun` builds `*rulebook.Rulebook` inline (empty maps, `DriftCosts: []int{1}`) rather than loading from a data directory. | The dry-run smoke needs no real asset definitions — `RepairFaction` and the other no-collector actions only inspect faction HP/stats. Loading a rulebook from disk would require a valid `FactionDataDir`, coupling the smoke to campaign data that a contributor running the binary cold may not have. The inline struct satisfies the engine's type contract without any IO. |
| 259 | `smokeHexRouter` is a no-op `world.HexRouter` stub; `NewSmokeWorldEngine` wraps it via `world.NewWithMap`. | `runMovementPhase` returns `ErrWorldEngineUnavailable` if `e.World == nil` — the nil check fires before eligibility is computed, so even a zero-asset faction would error out. With zero assets and zero bases, `RebuildIndex` and `TickMovementOrders` iterate empty maps and never call `spatialMap.Location`, so the stub's methods are unreachable in practice. The compile-time assertion `var _ world.HexRouter = smokeHexRouter{}` ensures interface drift is caught at build time. |
| 260 | `ErrActionNotImplementedInFoundation` renamed to `ErrActionNotImplemented` at pre-merge. | The original name embedded "Foundation" and an "Initiative 3" reference in both the identifier and the error message. Per project style, task and phase references rot as the codebase evolves and must not appear in identifiers or string literals. The stub struct name and package context already make the scope clear. |
| 261 | `Adapter.Stop()` race and `tui.Run` nil parameters deferred to F-005.2. | `close(eventCh)` panics if the engine goroutine is still emitting — a `select` with `default` does not protect against sends to a closed channel. Foundation is safe only because `Adapter.Run()` is never called (engine is nil). Similarly, `tui.Run(nil, nil, nil, log)` is safe only while no TUI code path reaches engine methods. Both become load-bearing hazards when F-005.2 wires real state; both require resolution before `Adapter.Run()` is first exercised. |
...
| # | Decision | Rationale |
|---|----------|-----------|
| 262 | Constructor pattern for the `cmd/gm-toolkit` cobra command tree: each command package exports a `New*Cmd()` function, and `NewRootCmd()` assembles the tree via explicit `AddCommand` calls. Package-level `var fooCmd` definitions and `init()`-driven `AddCommand` calls removed; flag state stored as locals captured by `RunE` closures. `faction` stays flat in `package main` rather than being promoted to its own subpackage. | The original cobra-init pattern spread wiring across six files and required a 7-line bridge file in `cmd/gm-toolkit/campaign.go` solely to register the campaign subpackage with `rootCmd`. Constructor pattern makes the full tree readable top-to-bottom in `NewRootCmd()`, removes init-time global mutation, and gives each command's flag vars fresh per-construction storage instead of process-lifetime persistence. Closure-local vars chosen over a per-command options struct as the smallest-diff option that still captures the per-construction-storage benefit; a struct would be warranted if/when commands grow enough that pure-function `runFoo(opts)` testability becomes valuable. `faction` kept flat because promoting it would add a directory and package for a single command — symmetry isn't worth that cost until faction grows subcommands. |
| 263 | `isKebabCase` moved from `cmd/gm-toolkit/campaign/helpers.go` to `internal/campaigns` as `func ValidateID(id string) error`; test moved alongside as `TestValidateID` in `internal/campaigns/campaigns_test.go`. CLI calls `campaigns.ValidateID` directly (both `create` and the `huh` wizard validator). `Scaffold`/`Register` do **not** call `ValidateID` internally. | Campaign-id validity is a rule the `campaigns` package owns — it defines `ManifestCampaign.ID`, scaffolds new campaigns, and registers them. Validation in the CLI layer left a layering gap: any non-CLI caller of `Scaffold`/`Register` would bypass the rule. Returning `error` instead of `bool` lets `huh.NewInput().Validate(...)` consume it directly and surface a specific message to the user. Defense-in-depth (calling `ValidateID` inside `Scaffold`/`Register`) skipped per YAGNI: one caller today, no present-day failure mode, and the relocation alone closes the layering concern. |
| 264 | `gm-toolkit campaign create` does three pre-flight checks before any filesystem or registry writes: (a) `--path <dir>` is interpreted as the *parent* directory and the campaign root is built as `<dir>/<id>`; (b) registry `Lookup(id)` errors out if the id is already registered (pointing at the registry file path so the user can clean a stale entry manually); (c) `os.Stat` on the target dir errors out if it already exists. Wizard prompts for "Parent directory" and the `--path` flag defaults to `$GM_TOOLKIT_HOME` (see D265). | The original `--path` semantics required the user to spell out both the parent and the id-named subdirectory (`--path ~/campaigns/test-camp` for `id=test-camp`), which is redundant — the id is already on the command line. Parent semantics let `--path ~/campaigns` work uniformly across ids. The pre-flight checks fix a real orphan-directory bug: `Scaffold` ran before `LoadRegistry`/`Register`, so a duplicate id would leave a freshly created directory on disk with no registry entry. Moving the registry check ahead of `Scaffold` means `Register` only runs in cases that already passed the in-memory `Lookup`, making the post-`Scaffold` `Register` call effectively infallible. Rollback-on-failure was considered and rejected as YAGNI given the pre-flight closes the realistic failure mode. Pre-flight checks live at the CLI layer rather than inside `Scaffold`/`Register` so the package contracts stay "do the thing"; "is this slot empty?" is an orchestration concern owned by the CLI. |
| 265 | Registry location moved from `~/.gm-toolkit/campaigns.toml` to `$GM_TOOLKIT_HOME/campaigns.toml`. `GM_TOOLKIT_HOME` env var is required for any command that calls `LoadRegistry`/`SaveRegistry`; missing-env errors at the seam with an actionable message (`export GM_TOOLKIT_HOME=<your-campaigns-dir>`). `create` defaults its `--path` flag and the wizard's "Parent directory" suggestion to `$GM_TOOLKIT_HOME`, so a configured user can run `gm-toolkit campaign create <id>` with no flags. Existing `~/.gm-toolkit/campaigns.toml` files are not migrated automatically. | The dotfolder location was hidden and inconvenient when the user wanted to inspect or hand-edit the registry. Co-locating the registry with the user's campaigns directory makes it discoverable (`ls ~/campaigns/` shows it next to the campaign dirs) and editable without spelunking into a hidden folder. Env var chosen over an auto-pointer file (`~/.gm-toolkit/home.toml`) because env var is explicit, Unix-idiomatic, and avoids introducing a meta-file whose own location and lifecycle would need governance. Required-not-optional because a silent fallback to `~/.gm-toolkit/` would defeat the point of the move — users would not notice they hadn't configured the env var and would still get a registry in the inconvenient location. Migration deferred per YAGNI: single user pre-merge, the file is one `mv` away. |
| 266 | `Scaffold()` derives its `MkdirAll` targets from `Paths()` rather than maintaining a parallel hard-coded list of subdirectories. An ephemeral `Campaign` is constructed before any filesystem writes, `Paths()` is called on it, and the results drive `MkdirAll`. The one literal `Scaffold` must add beyond what `Paths()` names is `filepath.Join(paths.FactionDataDir, "assets")` — a rulebook-format convention that campaigns cannot avoid knowing because `internal/campaigns` is upstream of `internal/faction/rulebook`. | `Scaffold` and `Paths` are co-located in the same package but were written in separate sessions and drifted: the old code had `rulebook/factions/` and `rulebook/spatial/` in `Paths()` while `Scaffold` was already creating different paths. Making `Scaffold` derive from `Paths()` means a single change to the layout method propagates to scaffolding automatically. |
| 267 | Campaign directory layout: `FactionDataDir = <root>/rulebook`, `SpatialDataDir = <root>/spatial`. Bundled rulebook source lives at `rulebooks/swn/` with asset TOML files in an `assets/` subdir and goals/tags/drift_costs flat at root. `rulebook.Load`'s `loadAssets` updated to glob `<dataDir>/assets/*_assets.toml`. The `testdata/duplicate_ids/` fixture reorganized to match. | The plan specified `rulebook/factions/` and `rulebook/spatial/` but the user's chosen source layout (`rulebooks/swn/assets/` for assets, `rulebooks/swn/` for the rest) and the already-updated scaffold code made those paths wrong. `FactionDataDir = rulebook/` keeps the top-level layout flat while grouping the multi-file asset glob under `assets/`. `SpatialDataDir = spatial/` is a sibling of `rulebook/`, not a child, matching the actual scaffold structure. |
| 268 | Test path resolution in `testharness/harness.go` (`testRulebookDir`) and `testharness/scenarios/helpers.go` (`testDataDir`) uses `runtime.Caller(0)` to derive the repo root from the source file's compile-time path rather than a relative path constant. | Relative constants (`"../../../data"`) resolve relative to the working directory at test invocation time, which is typically the package directory under `go test` but is not guaranteed. `runtime.Caller(0)` embeds the absolute source path at compile time, making resolution robust regardless of invocation context. Pattern is consistent across both packages so it is recognizable. |

<br />

### feature/spatial-map-cli — pre-merge checklist

| # | Decision | Rationale |
|---|----------|-----------|
| 269 | `dataErrorf` uses conditional prefix construction rather than format-string interpolation when `context` is empty. | The plan specified the prefix collapses to `data.toml:` for top-level TOML errors (no context selector). The original `"data.toml:%s: "+format` with `%s = ""` produced `"data.toml:: ..."` (double colon). Conditional branch on `context == ""` selects the correct prefix string at the cost of two extra lines; no format-string plumbing needed. |
| 270 | Same-region warps (both endpoints in the same region) are rejected with an error at warp-validation time. | A self-loop boundary (`ToRegion == owning region`) passes `selfCheck` silently — `LoadRegionMap` does not assert `boundary.ToRegion != region.ID`. Rejecting at the source is consistent with the fail-fast pipeline contract and prevents the engine routing graph from acquiring a phantom intra-region crossing edge that the GM almost certainly authored by mistake. |
| 271 | Warp deduplication extends the step-5 `seen map[edgeKey]bool` to cover step 6 rather than using a separate data structure. Duplicate or already-covered warp entries return an error. | Natural adjacency edges are deduplicated via `seen` in step 5; extending `seen` scope to step 6 at zero cost catches (a) two identical warp entries and (b) a warp whose hex pair is already covered by a natural adjacency edge. All three cases are GM authoring errors; fail-fast is consistent with the pipeline's general error contract. Silent deduplication was rejected because it would mask the mistake. |

<br />

### refactor/spatial-builder-design
...
| 275 | `spatial.RegionMap.AllWorlds() []Location` added to expose world iteration to the wizard. The `worlds` field is unexported; the plan's `worldsToOptions` helper required iterating all worlds to build the homeworld picker. A thin public method returning a slice of the `Location` interface keeps the world set opaque while giving the wizard what it needs. | The alternative was to export the `worlds` map directly, but that would expose `*World` pointer internals. Returning `[]Location` matches the existing `Location(id)` return type and stays behind the interface. |
| 276 | Faction ID generated by `slugify(name)` with `-2`, `-3` collision suffix; uniqueness checked against `factionState.Factions` inside `synthesize()`. | The plan recommended this approach (Open Question 3). User-typed IDs would require an extra input step; auto-increment (`F-001`) loses the human-readable link to the name. Slug-from-name is the best default: IDs stay readable in TOML, and collisions are rare in practice. The `generateID` loop is unbounded but terminates in any realistic faction count. |
| 277 | Asset-step shape: two `huh.MultiSelect` groups — one filtered to the faction's primary-attribute category, one for any attribute. Quota enforced per-group using `AssetCountsFromScale`. Options are dynamic (`OptionsFunc`) so they re-evaluate as scale and homeworld change in earlier steps. | Robert chose two MultiSelects over a single group (Open Question 2). The two-group shape matches SWN's "N primary + N any" wording, giving the GM a clear mental split. `OptionsFunc` is used rather than static slices because the eligible set depends on the primary stat, homeworld tech level, and scale — all of which are bound earlier in the form. |
| 278 | Detail view scrolls overflow through `bubbles/viewport` rather than clipping. The faction block is rendered once into the viewport via `SetContent`; `j/k`/arrows/pgup/pgdn scroll; `d`/`esc` are matched before forwarding to the viewport so delete/back take precedence over viewport's default half-page-`d`. | A tall faction was pushing the app header (and sometimes the mode bar) off the top of the altscreen — in an altscreen app the `View()` string *is* the frame, so emitting more rows than the terminal has clips from the top. Robert chose scrolling over height-clamp (which would silently hide content). Viewport keeps the chrome pinned and the whole faction reachable. |
| 279 | Permanent single-line help footer, bracketed by two `AppDivider` rules to mirror the app header. `?` toggles its visibility (reclaiming the 3 rows) rather than expanding a grid. Replaces the on-demand full-grid overlay (`help.ShowAll` set to `false`). | Robert wanted always-visible action hints framed like the header, single-line not grid. Keeping `?` as a visibility toggle preserves a way to reclaim screen space without contradicting the "permanent" default. |
| 280 | The root model owns the full vertical layout budget (`headerHeight = 5`, `footerHeight = 3`, `contentHeight()`), and forwards the computed content height to every sub-model via a synthetic `WindowSizeMsg` (in `resizeSubs`, called on terminal resize and on `?` toggle). `manage`'s `chromeHeight` constant was removed; sub-views fill the height they are handed. | Adding the footer made the chrome a two-package concern; leaving `manage` to subtract its own magic number would have let the two row-math sources drift. Centralizing at the frame owner gives a single source of layout truth and auto-pins the footer on the Spatial/Quit/confirm-exit screens too. The `?` toggle changes the content budget, so it must re-emit sizing through the model — a view-only toggle would leave the detail viewport sized wrong. |
| 281 | Root key routing uses a capability predicate: an `inputCapturer` interface with a single `CapturesInput() bool` method. The root `Update` checks whether the active sub satisfies `inputCapturer` and returns true before forwarding any key event; if it does, the root skips its own global bindings (`q`, tab, `?`) and routes the key directly to the sub. The `manage` model returns true when the create wizard is active. | The alternative was sub-first-refusal (root offers the key to the sub first; sub returns a bool signalling whether it consumed it). That would have changed every sub's `Update` signature or required a parallel `HandleKey` method — a broad contract change for an edge case only the wizard needs. The capability predicate is opt-in and requires zero changes to subs that don't capture input. |
| 282 | `CreateFaction` and `DeleteFaction` roll back their in-memory mutation if the subsequent `Save` call fails, leaving `FactionState` unchanged on any error return. | The prior contract said "no rollback; caller must retry" but the list was never rebuilt on error, making the in-memory state silently inconsistent with the on-disk state. A rollback on `Save` failure gives callers a clean guarantee: an error means nothing changed, so a non-rebuilt list is always truthful and a retry is safe without any additional caller logic. |
| 283 | `NewFaction` panics when the three chosen stats are not distinct, treating it as a programmer-error precondition rather than a returned error. | The sole caller is the create wizard, which already enforces distinctness through the form UI — the combination `(Attack, Attack, Defense)` cannot be submitted. Returning an error would require every call site to handle a branch that cannot occur in practice, adding noise without safety. A panic makes the violation loud during development without burdening the production call path. |
| 284 | The `contextstrip` package (`internal/faction/tui/views/manage/contextstrip/`) was deleted. | It was committed during early feature scaffolding but never wired into the view hierarchy. Its intended content — Campaign name, Cycle number, Faction count — already renders in the app header (`view.go`). Keeping the dead package would invite future confusion about where that chrome lives. |
...
| # | Decision | Rationale |
|---|----------|-----------|
| 285 | The execution overlay renders within the Center region (`lipgloss.Place(widths[Center], …)`) with the turn-order rail and detail card still rendering, rather than replacing all three panes full-screen. Revises plan Decision 11. | Plan Decision 11 had the mounted overlay replace the panes (matching the app's existing full-screen modal pattern), deferring the "modal within panes" look to Effort 2. In testing, the full-screen swap on every collector prompt flashed the three-pane view in and out on each phase, which Robert found jarring. Scoping the overlay to the center column keeps the rail and detail pinned so the layout holds steady. This is a `View`-only change — the dispatch seam (`CollectorAskMsg` → mount → keypress → `OverlayDoneMsg` → `Reply`) is untouched, so it carries no risk to the channel round-trip. The dimmed-panes composite (a real overlay drawn over greyed content) stays deferred to Effort 2, as it needs a string-splice compositing helper the codebase lacks. |
| 286 | The TUI launch path wires the world engine via `world.NewWithMap(spatialMap, log)`, reusing the region map `factionRun` already loaded; the structural fix is deferred to backlog `R-012`. | `factionRun` built the engine with `NewWithRulebook`, which leaves `Engine.World` nil; the movement phase then returned `ErrWorldEngineUnavailable` mid-cycle — only surfaced once Commit 7 let a cycle reach the movement phase. The minimal fix reuses the already-loaded `spatialMap` rather than re-reading the spatial dir via `world.New`. The underlying smell — `Engine.World` assigned post-construction, so a partially-wired engine compiles but fails deep in a cycle — is logged as `R-012` (review constructors so every user gets a fully-wired engine or fails fast at launch) rather than fixed inline, per YAGNI and to keep the bugfix scoped. |
| 287 | `execution.Update` forwards every unmatched message to the mounted overlay, and `turn.Update` batches the `execCmd` (carrying `overlay.Init()`) with `AskPump()` instead of discarding it. | `huh` forms dispatch internal setup commands (focus, nextField, etc.) via `Init()`; those commands must flow back through the Bubble Tea runtime and re-enter `Update` to reach the form. Without forwarding in `execution.Update`, the form stalled after init. Without batching in `turn.Update`, the Init cmd was dropped entirely because the `CollectorAskMsg` arm previously discarded `execCmd`. Both sides of the round-trip were needed. |
| 288 | The `ModeQuit` modebar tab is removed; quit is handled entirely through the `q` global keybinding and the `confirmExit` overlay. | The tab was navigable (Tab/Shift-Tab could land on it) but served no purpose — the `q` key triggered the confirm dialog independently, and `confirmExit` rendered the prompt over whichever tab was active. The tab just created a dead state the user could accidentally navigate into. Removing it also drops `priorMode` (previously used to restore the active tab on Esc, now unnecessary since the tab never changes). |
| 289 | The Center column is split vertically into a banner, a fixed `wizardBandHeight`-row wizard band holding the active overlay, a one-row divider, and a persistent event-stream viewport that takes the remaining height. Revises Decision 285 (overlay rendered *over* the center). | Under D285 the overlay replaced the stream's content via `lipgloss.Place` over the full Center region, so the stream was occluded by every modal and only reappeared in the gap between one overlay closing and the next opening — a flash, not a screen. Stacking the overlay and stream in different rows of the same column means they never compete for the same cells, so the stream is visible the whole turn. The band is sized fixed (14) rather than as a fraction because `huh` caps a `Select` at `defaultHeight = 10` visible options and scrolls beyond, so the tallest form (title + ≤10 options + footer) is ~13 rows regardless of campaign size; a percentage split would clip on short terminals and waste space on tall ones. The split lives entirely in `execution.View()` (`JoinVertical`); `layout.Compose` stays column-only. |
| 290 | A one-row phase banner tops the Center column, reading `▶ <Faction> · <Phase>`, with the phase anchored to the active modal (a genuine pause point) and reading `resolving…` between modals. | The phase the banner names is derived from the mounted overlay's `AskKind` (`m.activeAsk`, read only while `m.overlay != nil`), not from `m.phase`. `m.phase` is a *speculative prediction* of the next interactive phase (D-comment: "correct by the time that phase's modal opens") that churns through every value in a single event burst whenever the engine isn't parked on an ask; a banner reading it would flicker identically to the right-pane flash below. Anchoring to overlay presence means the banner text changes only at moments the user can actually read it. `AskAwaitCheckpoint` is the `iota` zero value, so `m.overlay != nil` (not a zero-sentinel) remains the single source of truth for "a modal is up." |
| 291 | The phase-reactive detail card (`detailView`) keys its specialized stat-raise / movement layouts off the **mounted modal** (`m.activeAsk`, read only while `m.overlay != nil`), not off turn events. The speculative `m.phase` field, the `turnPhase` enum, and the phase bookkeeping in `applyEvent` are removed as now-dead. | Two bugs, one cause: the card was driven by `m.phase`, a *speculative prediction* updated on phase-boundary events. (1) It flashed on the right pane between turns — events stream onto a buffered `eventCh` and the engine runs free between asks, so a burst of boundary events each drove a re-render and the card morphed stat-raise → movement → base faster than the eye. (2) The movement asset list silently vanished for any faction with **in-flight assets**: the engine ticks in-flight orders and fires `OnMovementTicked` *before* the movement ask (`orchestrator.go:335`, guarded by `len(tickMutations) > 0`, then `:338→647`), and `EvtMovementTicked` reset `phase = phaseNone`, so the modal opened with the card already advanced past movement. The mounted modal is the only ground-truth phase signal — keyed off it, the card is both flicker-free and correct regardless of tick ordering. The cargo modal reuses the movement card (`movables` still populated) so the asset list stays up while loading. With `detailView` no longer reading `m.phase`, the field and its enum were write-only and deleted. |
| 292 | `NewWithRulebook(rulebook, worldEngine, log)` is single-phase: it takes the world engine as a constructor parameter, assigns `Engine.World`, and calls `actions.RegisterDefaultActions` itself. The dead `New(paths, log)` constructor and `ErrSpatialDataDirRequired` are deleted; the four composition sites (`cmd/gm-toolkit/faction.go`, `tui/dryrun.go`, `testharness/harness.go`, `scenarios/spot_check_test.go`) drop their post-construction `eng.World = …` and manual `RegisterDefaultActions` lines. Resolves backlog R-012. | Action registration was a hoisted free function every composition site had to remember to call. The production TUI path (`cmd/gm-toolkit/faction.go`) never did — factions reached action selection with an empty registry — yet the bug stayed invisible because the test and dryrun fixtures all call it, so coverage was *anti-correlated* with catching the omission. Folding registration into the one constructor every site already calls makes a fully-wired engine the only constructible engine, which is exactly the R-012 mandate ("fully-wired or fail fast") that D286 deferred. Taking `World` as a parameter (rather than the prior post-construction assignment D286 flagged) also closes the nil-`World` window that produced the mid-cycle `ErrWorldEngineUnavailable`. The fallible `New()` had zero callers, so the two-phase construction it justified was dead code; deleting it removed the only rationale for late `World`. |
| 293 | `RegisterDefaultActions` takes its dependencies explicitly — `(ae *action.ActionEngine, resolveRoller func() domain.Roller, hookRegistry *hooks.Registry, worldEngine *world.WorldEngine, rulebook *rulebook.Rulebook)` — instead of `*engine.Engine`. The roller is passed as a thunk resolved at factory-invocation time; world, hooks, and rulebook are passed by value. | `engine` imports `action` (the `*action.ActionEngine` field), so `engine` cannot import the `actions` leaf package without a cycle. The sole edge creating that cycle was `RegisterDefaultActions`'s `*engine.Engine` parameter; replacing it with the concrete dependencies the factories actually read lets `engine` own registration without inverting the import direction (verified: `world`, `hooks`, and `action` none import `engine`). The roller stays a thunk because the test harness swaps `Engine.Rand` for a deterministic `FixedRoller` *after* construction (nine scenario sites); the prior closures read `e.Rand` lazily through the engine pointer, so capturing it by value silently froze the original random roller — `go build` could not catch it (the types still matched), only the dice-driven `Warlike`/`Fanatical` tag tests failed. `World` and `Hooks` are passed by value because nothing swaps them post-construction; the asymmetry deliberately encodes which fields are construction-fixed versus still-swappable seams. |
| 294 | Esc in the movement and cargo overlays declines with the typed empty answer (`[]world.MovementDecision{}` / `[]*domain.Asset{}`) emitted via `OverlayDoneMsg`; checkpoint and stat-raise keep swallowing Esc. Narrows Effort-3 plan Decision 2 (Esc-cancel scoped to action overlays only). | Pre-merge review flagged Esc as dead in these two overlays — indistinguishable from a frozen form, and inconsistent with the action overlays where Esc cancels. Unlike checkpoint (no empty answer) and stat-raise (decline is an explicit form option), movement and cargo have legal empty answers the orchestrator already handles (`len(decisions) == 0` skips movement, orchestrator.go:651; cargo doc states "nil/empty = no cargo"), so Esc can decline without invoking the recoverable-error cancel machinery. The answer must be the *typed* empty slice: `phase_collector` type-asserts the reply, so an untyped nil would fail the assertion and surface as a format error. |
