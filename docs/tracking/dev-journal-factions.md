...
- `AbilityAssetsModel` dual selection tracking (`tui/inputs/ability_assets.go`): `selectedSet map[int]bool` and `selectionOrder []int` are kept in sync on every toggle; `selectedSet` exists purely for O(1) membership checks in View; micro-optimization on a list of ≤10 items — if ever out of sync (future refactor), it would produce silent display bugs
