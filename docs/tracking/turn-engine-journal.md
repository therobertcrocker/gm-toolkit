...
| 2 | Per-Faction Bookkeeping | Complete | Income and maintenance skeleton done; maintenance costs deferred until AssetDefinition carries structured cost data; orchestrator owns Apply + Record + Save |
...
|---|----------|-----------------|
| 1 | ~~How does the Goal Engine communicate lock state back to the turn flow — method call, return value, or flag on `TurnState`?~~ Resolved: `GoalEngine.CheckLock` returns a `GoalLock` value (Option B); the orchestrator calls it before action selection and routes accordingly | Goal Engine |
...
| 3 | What does the new TUI implement for `TurnObserver`? The observer fires `OnBookkeepingApplied`, `OnActionResolved`, `OnCycleCompleted` etc. — the TUI needs to decide whether to display these inline or accumulate them for a summary view. Design question for the TUI-rebuild session. | TUI Rebuild |
